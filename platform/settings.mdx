...
1. **Provider-verified checks.** Integrations whose name matches a known provider are validated against that provider's live API using the credentials configured in the environment:
   - **Instantly** verifies the outbound connection over `REST API v2 (Instantly Outbound)`.
   - **ElevenLabs** calls the ElevenLabs user endpoint with `ELEVENLABS_API_KEY`. If the key is not set, the test fails with `ELEVENLABS_API_KEY is not configured in environment.`
   - **HubSpot** queries the CRM contacts endpoint with `HUBSPOT_PAT`. If the key is not set, the test fails with `HUBSPOT_PAT is not configured in environment.`
