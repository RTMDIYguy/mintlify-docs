...
Each registered webhook and MCP server has a **Test** action that runs a live handshake through the `settings.testIntegration` mutation. The test reports the protocol negotiated and the roundtrip latency:
...
```json
{
  ...
  "message": "Successfully verified handshake with PostgreSQL MCP (34ms roundtrip).",
  "timestamp": "2026-09-06T19:04:41.000Z"
}
```

- MCP integrations report `Model Context Protocol v1.0 (SSE/stdio)`.
- Webhook integrations report `REST Webhook / OAuth 2.0`.
