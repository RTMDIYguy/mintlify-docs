...
description: "Operate the URC agency from one authenticated cockpit: dispatch DAG workflows, monitor the agent swarm, clear the approval queue, and command the Orchestrator in natural language."
---

The Agency Command Center is the central operations cockpit for the Agent Lab platform. From a single page you can launch workflow runs into the execution engine, watch the status of every swarm agent, approve or reject runs that pause for human sign-off, and hand tasks to the Ops Orchestrator in plain English.
...
The terminal at the bottom of the page is a natural-language command line into the Orchestrator, powered by Google Gemini 2.5 Flash. Type an operational task and press **Send**. For example:
