...
- **Web server framework upgrade.** The Express web server framework behind the platform's API moved from 4.21 to 5.2. The upgrade tightens `res.status()` input validation, refreshes body parsing and static file serving, and adds deprecation warnings for redirects called with undefined arguments. You get more predictable API responses and a stronger foundation for future backend improvements, with no change to how the app looks or works day-to-day. See the [Quickstart](/quickstart) to get started.
