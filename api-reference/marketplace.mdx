...
Mounts the playbook identified by `:id` (for example, `sal-playbook`) into the current workspace. If a subscription record already exists, it is reactivated with status `active`; otherwise a new one is created.
