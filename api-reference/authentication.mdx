...
Agent Lab ships native authentication endpoints for email/password sign-up and sign-in, Google sign-in, session lookup, and logout. Successful authentication sets a session cookie that stays valid for one year. Endpoints also accept the session token as a `Bearer` token in the `Authorization` header.
...
<Note>
  The display name for an auto-provisioned account is derived from the email
  address. For example, `ada.lovelace@example.com` becomes "Ada Lovelace". Users
...
Returns `200` with the same `user` shape as sign-up and sets the session cookie.
...
- The session token is delivered as an HTTP cookie and is also accepted as a `Bearer` token in the `Authorization` header.
