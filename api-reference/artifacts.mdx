...
Refinement evaluates the original content, rewrites it with Gemini (`gemini-2.5-flash`) when a Google AI API key is configured (falling back to deterministic placeholder and filler cleanup otherwise), re-evaluates the result, and inserts a new artifact with an incremented `revisionVersion` and `parentArtifactId` set to the original. The original artifact is never modified.
