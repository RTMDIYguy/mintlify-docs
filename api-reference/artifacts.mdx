...
| `status` | Lifecycle state: `draft`, `scheduled`, `published`, `failed`, or `archived`. |
...
When a `post` artifact is set to `scheduled`, the [social publishing dispatcher](/platform/social-publishing) picks it up on the next queue tick, posts it to LinkedIn, Facebook, Instagram, or Threads based on `targetPlatform`, and writes the outcome back to the artifact:

- On success: `status = "published"` with `metadata.postId`, `metadata.postedAt`, and `metadata.platform`.
- On failure: `status = "failed"` with `metadata.errorMessage`, `metadata.platform`, and `metadata.lastDispatchAttempt`.
