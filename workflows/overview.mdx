...
description: "How Agent Lab's workflow kits are organized, versioned, and navigated by operators and buyers."
---
...
- **Buyers** should read the department-level series that matches their pain, then open the specific kit whose `trigger` matches when they would actually run it. The `automationLevel` field tells them how much of the workflow is automated versus manual.
- **Operators** should open `kit.md` first, work through Setup and Quickstart, and only dive into `source/`, `automation/`, or `trackers/` when the manifest points them there. Record every meaningful change through the change log in the manifest and the change control register.
- **Reviewers certifying a kit** should walk the Validation checklist in `kit.md`, confirm the manifest's `status` and `conformance` reflect reality, and split the URC/internal and client-facing variants before promoting the kit to Active or Certified.
...
The flagship is a Draft kit today. It is the first sellable cut while the underlying Marketing kits are still being converted, and it deliberately excludes paid advertising (MKT-07), social media management (MKT-08), and event marketing (MKT-09). Use it as a template for how future cross-series bundles should be packaged.
